...
function drawStatGridOnImage(image, profile) {
    ...
    const statBlocks = [
        ...
        {name: "Avg Objective Time", value: (profile.competitive.global.objective_time_average + 'm')},
        {name: "Win/Loss Ratio", value: wlRatio.toFixed(2)}
    ];

    return Promise.map(statBlocks, stat => createStatBlock(stat.name, stat.value.toString()))
        .map((statBlock, idx) => {
